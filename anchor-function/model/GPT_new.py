...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs):
        ...
        vab_embedding = self.tgt_emb(dec_inputs)
        pos_embedding = self.pos_emb(pos)
        dec_outputs = vab_embedding + pos_embedding # new
        ...
        dec_self_attns = []
        dec_output_each_layer = [vab_embedding, dec_outputs] # new
        for layer in self.layers:
            ...
            dec_output_each_layer.append(dec_outputs) # new

        # return dec_outputs, dec_self_attns
        return dec_outputs, dec_self_attns, dec_output_each_layer # new



class myGPT_new(nn.Module):
    def __init__(self, args, device):
        super(myGPT_new, self).__init__()
    ...
    def forward(self,dec_inputs):
        ...
        dec_outputs, dec_self_attns, dec_output_each_layer = self.decoder(dec_inputs)
        dec_logits = self.projection(dec_outputs)

        dec_logits_list = []
        for i in range(len(dec_output_each_layer)):
            dec_logits = self.projection(dec_output_each_layer[i])
            dec_logits_list.append(dec_logits.view(-1, dec_logits.size(-1)))
    
        return dec_logits.view(-1, dec_logits.size(-1)), dec_self_attns, dec_logits_list
    ...
    def greedy_decoder(self,dec_input):
        ...
        projected, _, projected_list = self.forward(dec_input)
        ...
        for i in range(len(projected_list)):
            projected = projected_list[i][-1,:].argmax()
            tmp_next_word = projected.item() 
            next_word_list.append(tmp_next_word)

        return next_word, next_word_list


    def test(self,sentence):
        dec_input = torch.tensor(sentence, dtype=torch.long, device=self.device).unsqueeze(0)

        output, output_list = self.greedy_decoder(dec_input)
