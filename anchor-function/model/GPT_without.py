...
class MultiHeadAttention(nn.Module):  # 多头注意力+残差拼接+层归一化都打包在这个函数里面了
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        # return output, attn  #去掉add&norm
        # return self.layernorm(output), attn # 去掉add
        # return output + residual, attn # 去掉norm

class PoswiseFeedForwardNet(nn.Module):   # 前馈层+残差拼接+层归一化
    def __init__(self, args):
    ...
    def forward(self, inputs):
        ...
        # return output  # 去掉add&norm
        # return self.layernorm(output)  # 去掉add
        # return output + residual  # 去掉norm
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs):
        ...
        dec_self_attn_mask = dec_self_attn_pad_mask # 去掉masked机制
...
class myGPT_without(nn.Module):
    def __init__(self, args, device):
        super(myGPT_without, self).__init__()
    ...
    '''
        dec_self_attns 是一个列表，其中包含每个解码器层生成的自注意力矩阵
        forward 函数的输出结果 dec_logits 是模型在给定输入 dec_inputs 下对每个词的预测分数。
        这些分数通常会经过 softmax 函数，将其转换为概率分布。在训练时，你可以使用这个概率分布计算损失，进行反向传播和优化
    '''

    def greedy_decoder(self,dec_input):
        # 在 greedy_decoder 函数中，使用 argmax 方法选择分数最高的词作为下一个词
