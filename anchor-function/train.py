...
# from torch.utils.data import DataLoader, TensorDataset #后续加入的语句
import os

# criterion 标准，规范
# def train_step(args, model, train_data_loader, optimizer, criterion, device, clip=1, scheduler=None):
def train_step(args, model, train_data_loader, optimizer, criterion, device, epoch, clip=1, scheduler=None):
    model.train()
    epoch_loss = 0
    for i, (dec_inputs, dec_outputs) in enumerate(train_data_loader):  
        ...
        # # 保存梯度的L2范数
        # if epoch % 20 == 0:
        #   gradient_file = f'{args.working_dir}/grad/gradient_info.txt'
        #   os.makedirs(os.path.dirname(gradient_file), exist_ok=True)  # 创建保存梯度文件的文件夹
        #   with open(gradient_file, 'a') as f:
        #       f.write(f"Epoch: {epoch}, Batch: {i}, Gradients:\n")
        #       for name, param in model.named_parameters():
        #           if param.requires_grad and param.grad is not None:
        #               grad_norm = torch.norm(param.grad)
        #               f.write(f"Parameter: {name}, Gradient L2 Norm: {grad_norm.item()}\n")                        
...
# def last_word_acc(model, data, seq_len, batch_size=32):
#     # 如果预测的最后一个词跟句子的最后一个词一样，视为预测正确
...
# def last_word_acc(model, data_loader, seq_len, device):
#     # 如果预测的最后一个词跟句子的最后一个词一样，视为预测正确
#     model.eval()
#     correct = 0
...
def train(args, datas):
    ...
    for epoch in range(args.n_epoch):
        ...
        train_loss = train_step(args, model, datas['train_data_loader'], optimizer, criterion, device, epoch, args.clip, scheduler=scheduler)
        ...
        # 修改保存acc的周期
        # if epoch <= 100 or epoch % args.print_acc_epoch == 0 or epoch == args.n_epoch-1 : #将acc的保存周期变成1
            acc_train, acc_test = [], []
            ...
            for i, data_name in enumerate(args.data_name):
                ...
                tmp_train_acc = last_word_acc(model, train_seq_group, args.seq_len)
                tmp_test_acc = last_word_acc(model, test_seq_group, args.seq_len)
        ...
        # 保存模型（覆盖式）
        if (epoch % args.save_model_epoch == 0 and epoch != 0) or epoch == args.n_epoch-1:
            torch.save(model.state_dict(), f'{args.working_dir}/model/model_{epoch}.pt')

        # 保存模型（精细化）
        # if (epoch % args.save_model_epoch == 0 and epoch != 0 and epoch>=0 and epoch<=30) or epoch == args.n_epoch-1:
            # torch.save(model.state_dict(), f'{args.working_dir}/model/model_{epoch}.pt')
        ...
        if ((epoch % args.plot_loss_acc_epoch == 0) and (epoch != 0)) or (epoch == args.n_epoch-1):
            ...
            if np.sum(args.data_show) != 0:
                plot_acc_of_each_data(args.working_dir, args.data_name, args.data_show)
