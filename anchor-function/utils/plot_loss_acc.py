...
def plot_acc_of_mask_unmask_data(working_dir):
    ...
    l1, = ax.plot(acc_epoch_his, acc_train_unmask_his, label=f'train (unmask)', lw=2, color=color_list[0], 
            marker = 'o', markersize=9, markeredgewidth=1, markeredgecolor='black', zorder=6)
    l2, = ax.plot(acc_epoch_his, acc_test_unmask_his, label=f'test  (unmask)', color=color_list[1], 
...
def plot_acc_of_each_data(working_dir):
    ...
    for k, index in enumerate(data_show_index):
        if args.data_mask[index] == 0:
            ...
            marker = '^'
        ax.plot(acc_epoch_his, train_acc_his[:, index], label=f'train ({args.data_name[index]})', color=data_color_list[k], \
                marker = marker, markersize=9, markeredgewidth=1, markeredgecolor='black', zorder=6)
        ax.plot(acc_epoch_his, test_acc_his[:, index], label=f'test  ({args.data_name[index]})', color=data_color_list[k], \
...
def plot_acc_of_mask_unmask_with_datasize_together(exp_dir, datasize_list, seed_list, target='3x_to_x'):
    r'''
        绘制最终实验结果和数据量的关系
        每个seed画一个多图
        多图中，每个子图画acc
    ...
    raw_num = math.ceil(len(datasize_list) / 2)
    ...
    for i, s in enumerate(seed_list):
        ...
        for j, N in enumerate(datasize_list):
            ...
            # legend
            if j == 0:
                ax.legend(loc=(-0.9, 0), handles=handles, frameon=False, handler_map=handler_map)
