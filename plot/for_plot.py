...
# 标准模型的绝对路径
# std_file_absolute_path = '/content/drive/MyDrive/anchor-function/anchor-function/result/GPT_3x_to_x_for_paper/forward_exp/1'
std_file_absolute_path = ''
...
for i in range(N_seed):
    ...
    loss_std=np.load(os.path.join(std_file_absolute_path, std_file,'loss', 'test_loss_his.npy'))
    loss_without_norm=np.load(exp_file+'/loss/test_loss_his.npy')
    # acc_std=np.load(std_file+'/loss/test_acc_his.npy')
    acc_std=np.load(os.path.join(std_file_absolute_path, std_file, 'loss', 'test_acc_his.npy'))
    ...
    # 精细绘图
    epochs_100 = list(range(0, 100))  # 1到100个epoch
    epochs_100_after = list(range(100, 1001, 100))  # 从200到1000，间隔为100
    x = epochs_100 + epochs_100_after
    # x=np.arange(0,11)*100  #acc如果每100轮算一次用这条
    axs2[1].set(xlabel='epoch', ylabel='test_acc', 	title=pic_title)
    # axs2[1].plot(x,acc_std,'o-',x,acc_without_norm,'o-')   #acc如果每100轮算一次用这条
    axs2[1].plot(x,acc_std,x,acc_without_norm) 
...
for i in range(N_seed):
    ...
    loss_std=np.load(os.path.join(std_file_absolute_path, std_file, 'loss', 'train_loss_his.npy'))
    loss_without_norm=np.load(exp_file+'/loss/train_loss_his.npy')
    # acc_std=np.load(std_file+'/loss/train_acc_his.npy')
    acc_std=np.load(os.path.join(std_file_absolute_path,  std_file, 'loss', 'train_acc_his.npy'))
...
for i in range(N_seed):
    ...
    # axs2[1].plot(x,acc_test,'o-',x,acc_train,'o-')  #acc如果每100轮算一次用这条
    axs2[1].plot(x,acc_test,x,acc_train)
...
for i in range(N_seed):
    ...
    loss_test=np.load(os.path.join(std_file_absolute_path,  std_file,'loss', 'test_loss_his.npy'))
    loss_train=np.load(os.path.join(std_file_absolute_path,  std_file, 'loss', 'train_loss_his.npy'))
    acc_test=np.load(os.path.join(std_file_absolute_path,  std_file, 'loss', 'test_acc_his.npy'))
    acc_train=np.load(os.path.join(std_file_absolute_path, std_file, 'loss', 'train_acc_his.npy'))
    
    x=np.arange(1,1001)
    axs2[0].set(xlabel='epoch', ylabel='loss', title=pic_title)
    axs2[0].plot(x,loss_test,x,loss_train) 
    axs2[0].legend(["test",'train'])
    axs2[0].grid()
    axs2[0].set_yscale('log')
